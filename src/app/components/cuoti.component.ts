...
/*import {shiyan}from'./mock-shiyan'*/
import {CommentService}from'./comment.service'
import {comms}from'./mock-comment'
import {qcomms}from'./mock-quescom'
declare const $: any;
@Component({
  providers: [CuotiService,CommentService],
  selector: 'my-cuoti',
  templateUrl: 'cuoti.component.html'
})

export class CuotiComponent implements OnInit {
  @Input()
  public cuoti:Cuoti;
  @Input()
  public id:string;
  public coms=comms;
  public coms2=qcomms;
/*  public shiyan=shiyan;*/
  public tags: Array<string>;
  constructor(private cuotiService: CuotiService,private commentService: CommentService) {
    this.tags = [];
    console.log(this.id);
  };
  ...
  /*chuxian(i:number){
    this.xiu=false;
    this.mcuo=this.cuotis[i].cuoyin;
  }*/
getTimu(): void {
    this.cuotiService.getCuoti(this.id).then(cuoti => this.cuoti=cuoti);
  };
getComs(): void {
  this.commentService.getComment(this.id).then(comms => this.coms=comms);
};
  ngOnInit(): void {
    /*this. getTimu();*/
   this.tags[0]=this.cuoti.cuoyin;
  };
  imgOptions1: Options = {
    // 自动上传。
    auto: true,
